...
- **No persistent backend**: Single-session demo; state resets on page refresh
- **No authentication**: No user accounts or session persistence
- **Cross-day memory is simulated**: In the demo, "memory" only exists within a single session
- **Safety layer is v0.1**: Not adversarially tested; needs clinical review before production use
- **No session length limit**: Conversations continue indefinitely (identified gap)
- **Rate limits**: Multiple concurrent users may experience slower responses due to API rate limits
